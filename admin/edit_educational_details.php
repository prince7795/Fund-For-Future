...
else
{
    ...
    if(isset($_GET['educational_id']))
    {
        $educational_id=str_replace("'", "\'", $_GET['educational_id']);
    }
    ...
    if(isset($_POST['academic_year']) && isset($_POST['marks_units']) && isset($_POST['result']))
    {
        ...
        {
            $query2="UPDATE `educational_details` SET `class`='$class', `academic_year`='$academic_year', `type`='$type', `extra_exam_appeared`='$extra_examinations', `marks_type`='$marks_type', `marks_units`='$marks_units', `result`='$result', `last_edited_by`='$admin_id', `last_edit_date`='$date' WHERE id='$educational_id'";
            $query2_run=mysqli_query($connect,$query2);
            if(mysqli_affected_rows($connect)>0)
            {
                $query21="INSERT INTO `system_logs`(`admin_id`, `module`, `action`, `affected_student`, `affected_kp`, `affected_decoding`, `affected_event`, `time`) VALUES ('$admin_id', 'educational details', 'edit', '$student_id', '', '', '', '$date')";
            ...
            else
            {
                $msg='<center><div class="alert alert-danger alert-dismissible" role="alert">
                          ...
                          Unable to update Educational details!
                        </div></center>';
            }
        }
        else
    ...
    $query3="select * from educational_details where id='$educational_id' and student_id='$student_id'";
    $query3_run=mysqli_query($connect, $query3);
    $educational_rows=mysqli_num_rows($query3_run);
            ...
            <div class="wrapper container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="panel panel-default">
                            <div class="panel-heading">Edit Educational details</div>
                ...
                <?php 
                    if($student_rows==1)
                    {
                        ...
                        {
                            if($educational_rows==1)
                            {
                ?>
                                <?php if(isset($msg)){echo $msg; } ?>
                                    <form class="form-horizontal" action="edit_educational_details?<?php echo $_SERVER['QUERY_STRING']; ?>" method="POST">
                                        <br/>
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-3">
                                                <select class="form-control chosen-select" id="type" name="type" onselect="hide_show();" onchange="hide_show();" onchange="hide_show();" data-placeholder="" required>
                                                    <option></option>
                                                    <option value="Academic" <?php if(!empty($existing_data['type']) && $existing_data['type']=='Academic'){ echo 'selected'; } ?>>Academic</option>
                                                    <!--<option value="Non-Academic" <?php if(!empty($existing_data['type']) && $existing_data['type']=='Non-Academic'){ echo 'selected'; } ?>>Non-Academic</option>-->
                                                </select>
                                            </div>
                                    ...
                                    ?>                                   
                                        ...
                                        <div id="selective_div" style="display: none;">
                                            ...
                                            <?php
                                                    ...
                                                    {
                                            ?>  
                                                            <option value="<?php echo $education_class['value']; ?>" <?php if(!empty($existing_data['class']) && $existing_data['class']==$education_class['value']){ echo 'selected'; } ?>> <?php echo $education_class['description']; ?> </option>
                                            <?php
                                                    }
                                            ?>                                   
                                                        </select>
                                    ...
                                    <?php
                                            ...
                                            {
                                    ?>  
                                                    <option value="<?php echo $career_option['value']; ?>" <?php if(!empty($existing_data['marks_type']) && $existing_data['marks_type']==$career_option['value']){ echo 'selected'; } ?>> <?php echo $career_option['description']; ?> </option>
                                    <?php
                                            }
                                    ?>                                   
                                                </select>
                                            </div>
                                            <label class="col-sm-2 control-label">GPA Obtained*</label>
                                            <div class="col-sm-3">
                                                <input type="text" class="form-control" name="marks_units" placeholder="Mark units obtained" value="<?= $existing_data['marks_units']; ?>" required>
                                            </div>
                                        </div>
                                        <hr class="dotted">
                                        <div class="form-group">
                                    ...
                                    <?php
                                            ...
                                            {
                                    ?>  
                                                    <option value="<?php echo $education_result['value']; ?>" <?php if(!empty($existing_data['result']) && $existing_data['result']==$education_result['value']){ echo 'selected'; } ?>> <?php echo $education_result['description']; ?> </option>
                                    <?php
                                            }
                                    ?>                                   
                                                </select>
                ...
                ?>
                                <center>
                                    <div class="alert alert-danger alert-dismissible" role="alert">
                                        <h3>No educational record found for this student.</h3>
                                    </div>
                                </center>
                <?php 
                            }
