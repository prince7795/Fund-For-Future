...
else
{
    ...
    {
        $address_id=str_replace("'", "\'", $_GET['address_id']);
    }
    ...
    if(isset($_POST['p_address']) && isset($_POST['p_city']) && isset($_POST['p_state'])
		&& isset($_POST['p_pin_code']) && isset($_POST['t_address']) && isset($_POST['t_city'])
		&& isset($_POST['t_state']) && isset($_POST['t_pin_code']))
    {
        $p_address=str_replace("'", "\'", $_POST['p_address']);
        //$p_landmark=str_replace("'", "\'", $_POST['p_landmark']);
        $p_city=str_replace("'", "\'", $_POST['p_city']);
        $p_tal_dist=str_replace("'", "\'", $_POST['p_tal_dist']);
        $p_state=str_replace("'", "\'", $_POST['p_state']);
        $p_pin_code=str_replace("'", "\'", $_POST['p_pin_code']);
        $t_address=str_replace("'", "\'", $_POST['t_address']);
        //$t_landmark=str_replace("'", "\'", $_POST['t_landmark']);
        $t_city=str_replace("'", "\'", $_POST['t_city']);
        //$t_tal_dist=str_replace("'", "\'", $_POST['t_tal_dist']);
        $t_state=str_replace("'", "\'", $_POST['t_state']);
        $t_pin_code=str_replace("'", "\'", $_POST['t_pin_code']);

        if(!empty($_POST['p_address']) && !empty($_POST['p_city'])
			&& !empty($_POST['p_state']) && !empty($_POST['p_pin_code'])
			&& !empty($_POST['t_address']) 
			&& !empty($_POST['t_state']) && !empty($_POST['t_pin_code']))
        {
            $query2="UPDATE `address_details`
					SET `present_address`='$p_address', `present_city`='$p_city',
					`present_state`='$p_state', `present_pincode`='$p_pin_code',
					`permanent_address`='$t_address', `permanent_city`='$t_city',
					`permanent_state`='$t_state', `permanent_pincode`='$t_pin_code',
            ...
            if(mysqli_affected_rows($connect)>0)
            {
                $query21="INSERT INTO `system_logs`(`admin_id`, `module`, `action`, `time`)
													VALUES ('$admin_id', 'address details', 'edit', '$date')";
            ...
            else
            {
                $msg='<center><div class="alert alert-danger alert-dismissible" role="alert">
                          ...
                          Unable to update Address details!
                        </div></center>';
            }
        }
        else
    ...
    $query3="select * from address_details where id='$address_id' and student_id='$student_id'";
        ...
        <section class="content">
            ...
            ?>
							...
							?>
                                ...
                                <form class="form-horizontal" action="edit_address_details?<?php echo $_SERVER['QUERY_STRING']; ?>" method="POST">
                                    <br/>
                                    <div class="form-group">
                                        <label class="col-sm-2 control-label">Present Address*</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" name="p_address" placeholder="Address" value="<?= $existing_data['present_address']; ?>" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-2 control-label">City*</label>
                                        <div class="col-sm-3">
                                            <input type="text" class="form-control" name="p_city" placeholder="City" value="<?= $existing_data['present_city']; ?>" required>
                                        </div>
                                    </div>
									<div>
									</div>
									<div class = "form-group">
                                        ...
                                        <div class="col-sm-3">
                                            <input type="text" class="form-control" name="p_state" placeholder="State" value="<?= $existing_data['present_state']; ?>" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-2 control-label">Postal Code*</label>
                                        <div class="col-sm-3">
                                            <input type="number" class="form-control" name="p_pin_code" min="0" placeholder="Pin Code" value="<?= $existing_data['present_pincode']; ?>" required>
                                        </div>
                                    </div>
                                    <hr class="dotted">
                                    <br/>
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" name="t_address" placeholder="Address" value="<?= $existing_data['permanent_address']; ?>" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-2 control-label">City*</label>
                                        <div class="col-sm-3">
                                            <input type="text" class="form-control" name="t_city" placeholder="City" value="<?= $existing_data['permanent_city']; ?>" required>
                                        </div>
                                    </div>
									<div>
									</div>
									<div class = "form-group">
                                        <label class="col-sm-2 control-label">State*</label>
                                        <div class="col-sm-3">
                                            <input type="text" class="form-control" name="t_state" placeholder="State" value="<?= $existing_data['permanent_state']; ?>" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-2 control-label">Postal Code*</label>
                                        <div class="col-sm-3">
                                            <input type="number" class="form-control" name="t_pin_code" min="0" placeholder="Pin Code" value="<?= $existing_data['permanent_pincode']; ?>"  maxlength="8" required>
                                        </div>
                                    </div>
                                    <hr class="dotted">
                                    <div class="form-group">
					...
					?>
                            <center>
                                <div class="alert alert-danger alert-dismissible" role="alert">
                                    <h3>No address record found for this student.</h3>
                                </div>
                            </center>
            <?php 
                        }
